 ...
 * `models/user.js`'s `getDocClient()` (spec 0-0-6-xray-downstream-tracing, task 7.4).
 *
 ...
 * `from()` is called with a SINGLE argument (no `marshallOptions`), preserving user.js's
 * original behavior across the lazy-getter conversion (tasks 7.1/7.3), and that the
 ...
 *
 * @module tests/unit/user-dao-xray
 */
...
/**
 * Mocks `@63klabs/cache-data` (DebugAndLog only — sufficient for user.js's needs) and
 * `../../config` so `require('../../models/user')` succeeds without pulling in the rest
 ...
 * >! @aws-sdk/lib-dynamodb's GetCommand/PutCommand/etc. construct against remain
...
describe('user.js X-Ray instrumentation', () => {
	it('enabled: wraps the RAW DynamoDBClient, and from() receives the wrapped client', () => {
		...
		UserDao.TestHarness.getInternals();
		...
		// >! Single argument only — user.js's original DynamoDBDocumentClient.from(client)
