...
const extendedDescriptions = {
  ...
  search_documentation: 'Search Atlantis documentation, tutorials, and code patterns by keyword. Returns results with title, excerpt, file path, GitHub URL, and result type. Requires the `query` parameter. Returns an empty array if no documents match the query. All filters are optional: `type` (`documentation`, `template-pattern`, `code-example`), `subType` (`guide`, `function`, `parameter`), and `ghusers` to narrow results to specific GitHub organizations. When results are broad, refine with `type` and/or `subType` rather than re-querying with more keywords. The response includes an optional `availableFilters` block listing the distinct `type` and `subType` values present in the matched set with their counts; use those values directly as filters on a follow-up query. When `totalResults` is large, `suggestions` also carries a narrowing hint.',

  get_document: 'Retrieve the complete source file behind a `search_documentation` result, rather than the excerpt. Supply exactly one lookup key: the `filePath` (contentPath) from a search result, or a section `hash` of 16 hexadecimal characters. A section-level key resolves to the file that contains it, so the response is the whole source file in document order. The response carries `filePath`, `githubUrl`, `repository`, `repositoryType`, `namespace`, and `content`. Retrieval is storage-only and the server never fetches from GitHub: when the requested document is not in storage, an error is returned that identifies the requested `filePath`/`hash` and includes the file-level `githubUrl` (or `null` when it cannot be derived) so you can fetch the file directly. Requires no elevated tier and works regardless of whether AI-assisted search is enabled. When the document is too large for a single response, a summary with `contentTruncated`, `totalChunks`, and a `retrievalHint` is returned instead; use `get_document_chunk` to read it incrementally.',

  get_document_chunk: 'Retrieve a specific chunk of a large document that was too large to return in a single `get_document` response. Takes the same lookup key as `get_document` (exactly one of `filePath` or `hash`) plus a zero-based `chunkIndex`. Returns `chunkIndex`, `totalChunks`, `filePath`, and the chunk `content`. Returns an error if `chunkIndex` is out of range or if the document is not in storage. Use this after receiving a truncated `get_document` response, requesting `chunkIndex` 0 through `totalChunks - 1` and concatenating the chunks.',
