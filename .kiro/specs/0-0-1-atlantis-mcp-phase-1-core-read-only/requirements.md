...
- **Bucket_Priority**: Order of S3 buckets to search, established by the order of buckets in the comma-delimited value passed to the CloudFormation template (and passed on to Lambda). The atlantis-mcp:IndexPriority tag is per-bucket for the Namespace within each bucket
...
4. WHEN atlantis_repository-type custom property is not set, THE MCP_Server SHALL exclude that repository from discovery
5. THE absence of atlantis_repository-type custom property SHALL be used to deliberately exclude repositories from MCP indexing
6. THE MCP_Server SHALL filter repositories by atlantis_repository-type when listing starters
7. THE MCP_Server SHALL filter repositories by atlantis_repository-type when searching documentation
8. THE MCP_Server SHALL document how organizations should set up custom properties on their repositories
9. THE MCP_Server SHALL require organizations to explicitly set atlantis_repository-type custom property for repository inclusion
10. THE MCP_Server SHALL log a warning when a repository is skipped due to missing atlantis_repository-type custom property
...
10. THE MCP_Server SHALL cache version lists using Cache_Data_Package with configurable TTL (default 60 minutes)
...
4. WHEN Sidecar_Metadata is not available, THE get_starter_info tool SHALL skip that starter and issue a warning
5. THE MCP_Server SHALL NOT fall back to GitHub repository metadata when Sidecar_Metadata is unavailable
6. THE absence of Sidecar_Metadata SHALL exclude the starter from discovery since the GitHub user/org cannot be determined
7. THE MCP_Server SHALL log a warning when a starter ZIP file is found without corresponding Sidecar_Metadata
8. THE MCP_Server SHALL document how to generate Sidecar_Metadata files during CI/CD deployment
9. THE documentation SHALL include a Python script for generating Sidecar_Metadata from GitHub repository
10. THE Python script SHALL be suitable for use in CodeBuild or GitHub Actions
11. THE Sidecar_Metadata SHALL include extracted information from GitHub custom property `atlantis_repository-type`
12. THE app starter ZIP file name SHALL match the GitHub repository name
...
10. THE get_starter_info tool SHALL prefer Sidecar_Metadata from S3 when available
11. WHEN Sidecar_Metadata is not available, THE get_starter_info tool SHALL skip that starter and log a warning
12. THE get_starter_info tool SHALL filter GitHub repositories by custom property `atlantis_repository-type: app-starter`
13. WHEN atlantis_repository-type custom property is not set, THE get_starter_info tool SHALL exclude that repository
14. THE get_starter_info tool SHALL return example code snippets from the starter repository when Sidecar_Metadata is available
...
2. FOR application resources (Lambda, DynamoDB, etc.), THE validate_naming tool SHALL validate pattern `<Prefix>-<ProjectId>-<StageId>-<ResourceName>` where all components are required
3. FOR S3 buckets, THE validate_naming tool SHALL validate pattern `<orgPrefix>-<Prefix>-<ProjectId>-<StageId>-<Region>-<AccountId>` where orgPrefix and StageId are optional
4. FOR S3 buckets, THE validate_naming tool SHALL also accept pattern `<orgPrefix>-<Prefix>-<ProjectId>-<Region>` (without AccountId) where orgPrefix is optional
5. THE validate_naming tool SHALL recognize that StageId is required in Application templates but optional in High-Level templates for shared resources
6. THE validate_naming tool SHALL verify Prefix (established in template.yaml)
7. THE validate_naming tool SHALL verify ProjectId (established in template.yaml)
8. THE validate_naming tool SHALL verify StageId matches allowed values (established in template.yaml)
9. THE validate_naming tool SHALL verify ResourceName follows AWS resource naming rules for the specified resource type
10. WHEN validation fails, THE validate_naming tool SHALL return specific error messages indicating which component is invalid
11. THE validate_naming tool SHALL provide suggestions for correcting invalid names
12. THE validate_naming tool SHALL validate names for S3 buckets, DynamoDB tables, Lambda functions, and CloudFormation stacks
13. THE validate_naming tool SHALL support validation of partial names (e.g., just Prefix-ProjectId)
...
9. THE MCP_Server SHALL cache version information using Cache_Data_Package with configurable TTL (default 60 minutes)
...
12. THE MCP_Server SHALL store all TTL values in settings.js within a ttl property (e.g., settings.ttl.fullTemplateContent)
13. THE TTL values SHALL be set from environment variables during deployment
...
4. WHEN GitHub API operations fail, THE MCP_Server SHALL log the repository, user/org, endpoint, and error details
5. THE MCP_Server SHALL return user-friendly error messages that do not expose internal implementation details
6. THE MCP_Server SHALL categorize errors as client errors (4xx) or server errors (5xx) using base response types from @63klabs/cache-data package
...
10. THE MCP_Server SHALL support configurable log levels (ERROR, WARN, INFO, DEBUG, DIAG) via environment variables using DebugAndLog from @63klabs/cache-data package
...
13. THE MCP_Server SHALL use DebugAndLog.info for informational messages about normal operations
14. THE MCP_Server SHALL use DebugAndLog.debug for detailed debugging information
15. THE MCP_Server SHALL use DebugAndLog.diag for diagnostic information
16. THE MCP_Server SHALL log which specific bucket/org failed for troubleshooting without exposing sensitive information
...
15. THE MCP_Server SHALL organize CloudFormation parameters in settings.js as properties and sub-properties for logical grouping
16. THE settings.js SHALL include organized sections for s3, github, cache, logging, and naming configuration
...
3. THE end-user documentation SHALL include integration guides for popular AI assistants (Claude, ChatGPT, Cursor, Kiro, Amazon Q Developer)
...
13. THE unit tests SHALL verify Sidecar_Metadata reading and exclusion when unavailable
14. THE unit tests SHALL verify GitHub custom property filtering and exclusion when property is not set
...
### Requirement 26: Lambda Function Directory Structure

**User Story:** As a developer, I want a clear directory structure that separates Lambda functions while keeping each function self-contained, so that the codebase is organized for future growth without premature abstraction.

#### Acceptance Criteria

1. THE MCP_Server SHALL organize Lambda functions in separate directories under `src/lambda/`
2. THE Read_Lambda SHALL be located in `src/lambda/read/` with all its code self-contained
3. THE Write_Lambda directory `src/lambda/write/` SHALL be created as a placeholder for Phase 2
4. THE Read_Lambda SHALL NOT depend on any shared code directories or Lambda Layers in Phase 1
5. THE buildspec.yml SHALL build and package the Read_Lambda from `src/lambda/read/` directory
6. THE buildspec.yml SHALL ensure only essential node_modules and code are deployed
7. THE template.yml SHALL define the Read_Lambda resource with CodeUri pointing to `src/lambda/read/`
8. THE template.yml SHALL include a commented-out Write_Lambda definition for Phase 2
9. THE directory structure SHALL follow this pattern:
```
...
└── template.yml
```
10. THE buildspec.yml SHALL run tests before building Lambda package
11. THE Read_Lambda SHALL import modules using relative paths (e.g., `require('./config')`)
12. THE Read_Lambda package.json SHALL include all dependencies including @63klabs/cache-data
13. THE tests SHALL be organized at the repository root level, not within individual Lambda functions
14. WHEN Phase 2 is implemented, shared code MAY be extracted to a Lambda Layer if duplication becomes significant
15. THE MCP_Server SHALL NOT prematurely create shared directories or layers until multiple functions actually require them