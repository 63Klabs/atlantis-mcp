...
| `DocAiAssistModel` | `us.amazon.nova-micro-v1:0` | Bedrock small model **invoke ID** used only in `semantic-assisted` mode for re-ranking; passed directly to the Lambda runtime as the `InvokeModel` `modelId`. Defaults to the Geo-US cross-region inference-profile ID for Amazon Nova Micro. May instead be set to a plain foundation-model ID (e.g. for strict in-region invocation). Public model identifier, not a secret. |
| `DocAiAssistFoundationModel` | `amazon.nova-micro-v1:0` | Plain Bedrock foundation-model ID that `DocAiAssistModel` routes to when it is a cross-region inference profile. Used only to build the destination-region IAM grant (see `DocAiAssistProfileRegions` below); ignored when `DocAiAssistProfileRegions` is empty. Public model identifier, not a secret. |
| `DocAiAssistMaxCandidates` | `25` | Maximum number of top candidates passed to the assist model for re-ranking. |
| `DocAiAssistProfileRegions` | `us-east-1,us-east-2,us-west-2` | List of AWS regions the cross-region **assist** inference profile named by `DocAiAssistModel` may route to. Used only for IAM `Resource` scoping — it is never passed to the Lambda runtime. Non-empty by default (Geo-US) so the out-of-the-box inference-profile `DocAiAssistModel` is authorized to route to each listed region: the assist IAM policy grants the inference-profile ARN plus `DocAiAssistFoundationModel` restricted to exactly these regions via the `aws:RequestedRegion` condition key. AWS performs the cross-region routing server-side. Set to `""` (empty) — and set `DocAiAssistModel` to a plain foundation-model ID — to fall back to the single plain in-region foundation-model ARN, matching the original pre-cross-region behavior. |
...
- The **assist model** (`DocAiAssistModel` / `DocAiAssistFoundationModel`, Amazon Nova Micro by default) — required only for `semantic-assisted` mode. Because `DocAiAssistModel` defaults to a Geo-US cross-region inference profile, request access to Amazon Nova Micro in each region listed in `DocAiAssistProfileRegions` (default `us-east-1`, `us-east-2`, `us-west-2`), not just the deployment region.
...
> **Note (cross-region model access):** cross-region Bedrock access is built in and works differently for each model type. The **embedding** model uses a hard client-side region pin: set `DocAiEmbeddingRegion` to source it from another region (embedding models cannot use inference profiles). When set, the embedding model's least-privilege IAM `Resource` ARN is built with that region instead of the deployment region. The **assist** model uses AWS server-side cross-region routing: `DocAiAssistModel` names a cross-region inference-profile ID (Geo-US by default) and `DocAiAssistProfileRegions` lists the regions it may route to (Geo-US by default). The assist IAM policy is then scoped to the inference-profile ARN plus `DocAiAssistFoundationModel` (the plain foundation model the profile routes to) clamped to exactly those regions via the `aws:RequestedRegion` condition key; the assist client itself needs no region override because routing is server-side. Set `DocAiAssistProfileRegions=""` and `DocAiAssistModel` to a plain foundation-model ID to fall back to the single plain in-region foundation-model ARN.
